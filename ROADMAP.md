...
The first push of this change failed CI on the Firefox and WebKit matrix legs (Chromium passed) - `-n 4`/`-n 2` had never actually exercised these three files together before, since they'd only ever been run individually or on Chromium locally. Reproduced both failures locally rather than guessing:
- **WebKit**: `test_filtering_the_catalog_via_the_department_dropdown` (and, by the same defect, `assert_alphabetical_order`/`assert_price_order`) called `.all_inner_texts()` as a one-shot synchronous read immediately after a step that triggers a page navigation (clicking "Filter"/"Sort", or a direct `page.goto`). `.all_inner_texts()` has no auto-retry, unlike `expect(...)` assertions, so it can be executed while the old page is still being torn down - Chromium/Firefox's timing happened to never expose this, WebKit's did (`Execution context was destroyed, most likely because of a navigation`). Fixed by adding an `expect(locator.first).to_be_visible()` wait immediately before each `all_inner_texts()` call in `steps/test_catalog_filter_sort_steps.py`, forcing the read to wait for the new page to have real content before taking the snapshot.
- **Firefox** (intermittent, not on every run): the two `otp_login.feature` scenarios both request/verify a one-time code for the same seeded email (`user2@example.com`), and `Otp.create()` deletes any prior pending code for that email before issuing a new one. Under `-n` parallel execution the two scenarios can land on different xdist workers and run concurrently, so one scenario's fresh code request can invalidate the other's already-retrieved code mid-verification - a test-isolation race, not an app bug (a real user opening two tabs and requesting two codes for the same account would see the identical "only the latest code works" behavior, by design). Fixed the same way the `manage_product.feature` CRUD chain already handles this class of problem: tagged both scenarios `@xdist_group_otp_login`, mapped via `pytest_bdd_apply_tag` in `conftest.py` to `pytest.mark.xdist_group(name="otp_login")`, pinning them to the same worker so they never run concurrently.

Both fixes verified with 3+ repeated full-suite runs (`-n 2`/`-n 4`, matching CI exactly) on all three browsers before re-pushing. The re-push still failed CI once more on Firefox, but on a *different* scenario (`test_successfully_delete_an_existing_product`, part of the `manage_product.feature` CRUD chain) - 5 further repeated local Firefox runs reproduced this exactly once, matching this project's own already-documented, pre-existing `-n`-related CRUD-chain flakiness (not a new regression from this track; the other 4 runs and every catalog/OTP scenario across all 5 were clean). Adding 3 more files to the same parallel run does mean marginally more concurrent browser load against the single-instance app, which may nudge that pre-existing flake's odds slightly - not something addressed here, consistent with how this risk has been treated everywhere else in this project (accepted, not chased).
